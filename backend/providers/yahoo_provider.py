...
class YahooProvider(BaseProvider):
    ...
    def validate_history(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df_safe[['date', 'open', 'high', 'low', 'close', 'volume']]
    ...
    def get_history(self, symbol: str, start_date: str, end_date: str) -> Optional[pd.DataFrame]:
        ...
        cache_key = f"history_{start_date}_{end_date}"
    ...
    def get_actions(self, symbol: str) -> Optional[pd.DataFrame]:
        def fetch() -> Optional[pd.DataFrame]:
            return self.safe_dataframe(self._ticker(symbol).actions)
        return self._fetch_cached(symbol, "actions", fetch)
    ...
    def get_share_holders(self, symbol: str) -> Dict[str, Any]:
        def fetch() -> Dict[str, Any]:
            ticker = self._ticker(symbol)
            return {
        ...
        return self._fetch_cached(symbol, "shareholders", fetch) or {}
    ...
    def get_fundamentals(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "roce": None,
            "roic": None,
            "roa": self.safe_float(info.get("returnOnAssets")),
    ...
    def get_financials(self, symbol: str) -> Dict[str, Any]:
        info = self._safe_info(symbol)
        return {
            "income_statement_annual": self.normalize_income_statement(self.get_income_statement(symbol)),
            "income_statement_quarterly": self.normalize_income_statement(self.get_quarterly_income_statement(symbol)),
            "balance_sheet_annual": self.normalize_balance_sheet(self.get_balance_sheet(symbol)),
            "balance_sheet_quarterly": self.normalize_balance_sheet(self.get_quarterly_balance_sheet(symbol)),
            "cashflow_annual": self.normalize_cashflow(self.get_cashflow(symbol)),
            "cashflow_quarterly": self.normalize_cashflow(self.get_quarterly_cashflow(symbol)),
            "currency": self.safe_str(info.get("financialCurrency")),
        ...
        if inst is not None and not inst.empty:
            for _, row in inst.iterrows():
                results.append({
                    ...
                    "percent_held": self.safe_float(row.get("% Out")),
                    "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                })
        return results
    ...
    def normalize_recommendation_summary(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df_safe.iterrows():
            results.append({
                "period": self.safe_str(row.get("period", row.get("index"))),
                "strong_buy": self.safe_int(row.get("strongBuy")),
                "buy": self.safe_int(row.get("buy")),
                "hold": self.safe_int(row.get("hold")),
                "sell": self.safe_int(row.get("sell")),
                "strong_sell": self.safe_int(row.get("strongSell")),
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            })
        return results
